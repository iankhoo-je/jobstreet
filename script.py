...
conn = sqlite3.connect('C:/Users/Ian/Documents/test_data.db')
c = conn.cursor()
c.execute('''CREATE TABLE proper_now
...
def general():
    ...
    for idx, link in enumerate(a_tag_lst):
        ...
        # c.execute('''SELECT role FROM proper WHERE role=?''',
        #                 (company_role))
        c.execute(
        "SELECT role, COUNT(*) FROM proper_now WHERE role = %s GROUP BY role",
        (company_role,)
            )
        ...
        if row_count == 0:
            c.execute("INSERT INTO proper_now VALUES (:role,:company,:location,:area,:salary,:years,:keywords,:description,:date)", 
    ...
    c.execute("SELECT COUNT(*) FROM proper_now")
    result = c.fetchone()[0]
    print(result)
    conn.commit()
    df = pd.read_sql_query("SELECT * FROM proper_now", conn)
